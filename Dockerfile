...
ENV TZ=Asia/Tehran \
    ADDITIONAL_PACKAGES="php7-pdo_pgsql php7-pdo_mysql php7-redis php7-curl php7-fileinfo php7-gd php7-pdo_sqlite php7-sqlite3" \
...
RUN apk update && \
    ...
    sed -i "/memory_limit = /c memory_limit = ${PHP_MEMORY_LIMIT}" \
           /etc/php7/php.ini && \
    sed -i "/upload_max_filesize = /c upload_max_filesize = ${PHP_UPLOAD_MAX_FILE_SIZE}" \
           /etc/php7/php.ini && \
